...
function EpisodeBuffer(num_envs::Int, state_size::Tuple, num_actions::Int,initial_horizon::Int = 10)
    states = zeros(Int8, state_size..., initial_horizon, num_envs)
    masks=zeros(Int8,num_actions,initial_horizon,num_envs)
    policy=zeros(Float32,num_actions,initial_horizon,num_envs)
    rewards = zeros(Float32, initial_horizon, num_envs)
    switches = zeros(Bool, initial_horizon, num_envs)
    ep_lengths = zeros(Int, num_envs)
    return EpisodeBuffer(num_actions,states, masks,policy, rewards, switches, ep_lengths)
end
...
function _increase_horizon(ep_buff)
    ...
    new_states = zeros(Int8, state_dim(ep_buff)..., new_horizon, num_envs(ep_buff))
    new_states[.., 1:current_horizon, :] .= ep_buff.states[.., 1:current_horizon, :]

    new_masks = zeros(Int8, ep_buff.num_actions,new_horizon,num_envs(ep_buff))
    new_masks[:,1:current_horizon, :] .= ep_buff.masks[:,1:current_horizon, :]
...
function save!(ep_buff, states::AbstractVector, masks, policy,rewards, switches)
    ...
    map(1:num_envs(ep_buff)) do env_id
        ...
        ep_buff.masks[..,save_index, env_id] .= masks[env_id]
...
function ReplayBuffer(max_size::Int, state_size::Tuple, num_actions::Int)
    states = zeros(Int8, state_size..., max_size)
    masks=zeros(Int8,num_actions,max_size)
    policy=zeros(Float32,num_actions,max_size)
    values = zeros(Float32, 1, max_size)
    return ReplayBuffer(max_size, num_actions, states, masks,policy, values, 0, 0)
end
...
function _overwrite!(rp_buff, rp_buff_range, ep_buff, ep_buff_range, env_id)
    ...
    rp_buff.masks[..,rp_buff_range] .= ep_buff.masks[..,ep_buff_range, env_id]

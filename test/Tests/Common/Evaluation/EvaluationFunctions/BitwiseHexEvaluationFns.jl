...
function act_minimax(single_env_vec, kwargs)
    ...
    return minimax(env; depth, eval_fn=hex_eval_fn, is_maximizing)
end
...
function get_hex_benchmark_fns(kwargs, metrics)
    ...
    function add_info_to_env_str(env, a, opt, idx)
        ...
        curplayer_row, hex_rows = rows[1], rows[3:end]
        info = "Action chosen: $a. Optimal Actions: $opt"
        fixed = "\n$curplayer_row\n" *
                "             " * hex_rows[1] * "\n" *
                "State $idx:  " * hex_rows[2] * "\t$info\n" *
                "             " * hex_rows[3] * "\n"
        return fixed
    end
